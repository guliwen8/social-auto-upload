...
DOUYIN_DEBUG_SCREENSHOT_PATH = Path("cookies/douyin_publish_debug.png")
...
def _build_chromium_launch_kwargs(headless: bool) -> dict:
    launch_kwargs = {
        "headless": headless,
...
async def cookie_auth(account_file):
    async with async_playwright() as playwright:
        ...
        browser = await playwright.chromium.launch(**_build_chromium_launch_kwargs(headless=False))
        try:
...
async def douyin_cookie_gen(
    ...
    async with async_playwright() as playwright:
        browser = await playwright.chromium.launch(**_build_chromium_launch_kwargs(headless=headless))
...
class DouYinBaseUploader(BaseVideoUploader):
    ...
    async def set_schedule_time_douyin(self, page, publish_date):
        ...
        await asyncio.sleep(1)
        schedule_input = page.locator('.semi-input[placeholder="日期和时间"]')
        ...
        try:
            await schedule_input.fill(str(publish_date_hour))
        except Exception:
            await schedule_input.evaluate(
                """(el, value) => {
                    el.value = value;
                    el.dispatchEvent(new Event('input', { bubbles: true }));
                    el.dispatchEvent(new Event('change', { bubbles: true }));
                }""",
                str(publish_date_hour),
            )
        await page.keyboard.press("Enter")
        await page.keyboard.press("Escape")
        await asyncio.sleep(1)

    async def dismiss_publish_blockers(self, page: Page) -> None:
        for _ in range(3):
            try:
                know_buttons = page.get_by_role("button", name="我知道了")
                count = await know_buttons.count()
                for index in range(count):
                    btn = know_buttons.nth(index)
            ...
            try:
                await page.evaluate(
                    "() => document.querySelectorAll('.shepherd-element,.shepherd-modal-overlay-container,[class*=\"popover\"],[class*=\"tooltip\"]').forEach(e => e.remove())"
                )
            except Exception:
                pass
...
class DouYinVideo(DouYinBaseUploader):
    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        browser = await playwright.chromium.launch(**_build_chromium_launch_kwargs(headless=self.headless))
        context = await browser.new_context(
        ...
        if self.publish_strategy == DOUYIN_PUBLISH_STRATEGY_SCHEDULED and self.publish_date != 0:
            ...
            await self.dismiss_publish_blockers(page)
        ...
        while True:
            try:
                ...
                await self.dismiss_publish_blockers(page)
                button_names = ["定时发布", "发布"] if self.publish_strategy == DOUYIN_PUBLISH_STRATEGY_SCHEDULED else ["发布", "定时发布"]
                publish_button = None
                for button_name in button_names:
                    candidate = page.get_by_role("button", name=button_name, exact=True)
                ...
                if self.debug:
                    try:
                        ...
                        douyin_logger.info(_msg("🧪", f"当前按钮状态: {button_texts}"))
                    except Exception as btn_exc:
                        douyin_logger.warning(_msg("🧪", f"读取按钮状态失败: {btn_exc}"))
                    try:
                        page_text = await page.locator("body").inner_text(timeout=3000)
                        douyin_logger.info(_msg("🧪", f"页面提示摘录: {page_text[:500]}"))
                    except Exception as text_exc:
                        douyin_logger.warning(_msg("🧪", f"读取页面提示失败: {text_exc}"))
                await asyncio.sleep(0.5)
