            ...
            for frame in page.frames:
                try:
                    if "login-for-iframe" in frame.url:
                        ...
                        if "微信扫码登录 视频号助手" in frame_text or "已扫码" in frame_text or "二维码已过期" in frame_text:
                            tencent_logger.info(_msg("🥹", "检测到视频号登录 iframe，当前仍未真正登录"))
                            return False
                except Exception:
                    continue
...
async def _is_tencent_login_completed(page: Page) -> bool:
    ...
    trusted_logged_in_urls = (
        page.url.startswith(TENCENT_UPLOAD_URL)
        or page.url.startswith(TENCENT_MANAGE_URL)
        or page.url.rstrip("/") == TENCENT_LOGIN_URL
        or page.url.startswith(f"{TENCENT_LOGIN_URL}/")
    )
    return trusted_logged_in_urls
...
class TencentBaseUploader(BaseVideoUploader):
    ...
    async def upload_video_file(self, page: Page, file_path: str) -> None:
        async def find_file_input():
            # 主 frame + 所有 iframe（视频号编辑器可能在 iframe 内）
        ...
        async def wait_for_file_input(timeout_ms: int = 30000):
            # 首选：直接等 input[type=file] 出现（页面“初始化中”时会延迟挂载）
            try:
                loc = page.locator('input[type="file"]').first
                await loc.wait_for(state="attached", timeout=timeout_ms)
                return loc
            except Exception:
                return await find_file_input()

        fi = await wait_for_file_input(timeout_ms=45000)
        if fi is None:
            # 兜底：有些场景 input 只有在点击上传区域后才会挂载
            click_candidates = [
                # 上传卡片（包含 + 号的虚线框）
                'div:has-text("上传时长")',
                'div:has-text("格式为MP4")',
        ...
        if fi is None:
            # 旧版助手落在首页：先点「发表视频」唤出编辑器与上传控件（保留兼容）
            try:
                ...
                pass
            fi = await wait_for_file_input(timeout_ms=30000)
