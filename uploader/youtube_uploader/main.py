    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            if "accounts.google.com" in url or "/signin" in url.lower():
                return False
            # 有些账号/地区进入 Studio 后不会立刻跳到 /channel/ 形态，
            # 只要已经进入 studio.youtube.com 且未被踢回登录页，就认为 cookie 有效。
            return "/channel/" in url or url.startswith(STUDIO_URL)
        except Exception:
            return False
        finally:
            await browser.close()
...
class YouTubeVideo(BaseVideoUploader):
    ...
    async def upload(self, playwright: Playwright) -> None:
        # YouTube Studio 在 headless 模式下容易出现“不支持的浏览器/版本过旧”提示，
        # 导致上传页无法渲染出 input[type=file]。
        if self.headless:
            youtube_logger.warning(_msg("⚠️", "YouTube 上传不建议使用 headless；已强制改为 headed 模式执行"))
        browser = await playwright.chromium.launch(
            headless=False,
            channel="chrome",
            proxy={"server": YT_PROXY} if YT_PROXY else None,
