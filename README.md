...
`social-auto-upload` 是一个面向创作者、运营者和自动化工作流的多平台发布工具。它同时提供：

- 各平台浏览器自动化 `uploader`
- 统一的 `sau` CLI
- 最小可用的平台控制面（账号、草稿、计划、任务执行、状态回写）

当前主线已经打通并完成真实验收的平台包括：
...
其中，`视频号` 和 `YouTube` 已完成“控制面创建计划 → 执行任务 → 真实上传 → 成功回写”的闭环验证。

<img src="media/show/tkupload.gif" alt="tiktok show" width="800"/>

## 当前状态

如果你是第一次进入这个仓库，先看这 4 个入口就够了：

- [安装说明](./docs/install.md)
- [CLI 使用说明](./docs/CLI.md)
- [发布前检查清单](./发布前检查清单.md)
- 验收总结：
  - [腾讯视频平台执行闭环验收总结](./tencent-acceptance/tencent-acceptance.html)
  - [YouTube 平台执行闭环验收总结](./youtube-acceptance/youtube-acceptance.html)

如果你是想直接跑主线，请优先使用：

- `sau_cli.py` / `sau`
- `sau_platform_server.py`
- `sau_platform_worker.py`

旧的 `examples/`、`myUtils/`、部分历史 Web 代码仍保留，但不再是当前主线。
...
| YouTube | ✅ | ✅ | ❌ | ❌ | ✅ | ❌ | 浏览器自动化（Studio），已完成平台控制面执行闭环验收 |
...
下面这些能力已经在本地真实环境里验证通过，不是只停留在代码层：
...
- 已修复登录页 `iframe` 误判
- 已修复上传页初始化慢导致“找不到上传框”
- 已放开控制面手动执行对历史 `cooldown / circuit_open` 的死挡

### YouTube

- 交互式 Google 登录与 cookie 落盘
- 控制面创建发布计划
- 控制面手动执行任务
- 真实上传并成功发布（`unlisted`）
- 任务状态回写 `succeeded`
...
- YouTube 上传已强制使用 `headed` 模式，避免 Studio 在 `headless` 下出现 “unsupported browser”
- 如网络环境需要代理，可设置 `YT_PROXY`
...
如果你只是想稳定跑起来，建议按下面顺序看：

1. [安装说明](./docs/install.md)
2. [CLI 使用说明](./docs/CLI.md)
3. [发布前检查清单](./发布前检查清单.md)
4. 平台验收总结：
   - [腾讯视频平台执行闭环验收总结](./tencent-acceptance/tencent-acceptance.html)
   - [YouTube 平台执行闭环验收总结](./youtube-acceptance/youtube-acceptance.html)
...
当前抖音、快手、小红书、Bilibili、视频号、YouTube 已接入 CLI：
...
sau tencent upload-video --account <account_name> --file videos/demo.mp4 --title "示例标题" --desc "示例简介" --tags 标签1,标签2 --schedule "2026-06-30 18:30"
...
> 当前主线已支持从环境变量覆盖：`export YT_PROXY="http://127.0.0.1:7890"`

> 腾讯视频说明：CLI 与平台控制面都已完成真实执行闭环验证。若页面没有“原创声明”入口，当前策略是记录诊断后继续发布，不阻塞任务。
